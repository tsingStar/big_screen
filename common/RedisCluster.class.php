...
class RedisCluster{
   ...
   * @return boolean
   */
  public function connect($config=array('host'=>'127.0.0.1','port'=>6379,'pwd'=>'','persist'=>true), $isMaster=true){
    // default port
    ...
    if($isMaster){
      ...
      $auth=$this->_linkHandle['master']->auth($config['pwd']);
      // echo var_export($a)
    }else{
      // 多个 Slave 连接
      ...
      $this->_linkHandle['slave']->auth($config['pwd']);
      ++$this->_sn;
    }
    return $ret;
  }
  ...
  public function get($key){
    ...
    if(! $this->_isUseCluster){
      // if($key=='xc_wxmiao_com_prize_win_lock_cache'){
      //   echo var_export(unserialize($this->getRedis()->dump($key)));
      // }
